...
while(<PRS>){
	chomp;
	if($_=~/\:/ && ($_=~/MISPCT/ or $_=~/PVALUE/ or $_=~/GQ/ or $_=~/HETEROZYGOUS_DEPTH/ or $_=~/HOMOZYGOUS_DEPTH/)){
...
my %tmp1;
my %hash;
my @parent_id=glob("*_$inputfile\_loc.parentgt");
my @Parent_Name=split/\_/,$parent_id[0];
my @progeny_id=glob("*_$inputfile\_loc.gt");
...
my $t="@progeny_id";
$t=~s/ /\t/g;
$t=~s/\_progeny_$inputfile\_loc.gt//g;
...
my @abxaa_line1=split/\s+/,$abxaa_line;
my @abxcc_line1=split/\s+/,$abxcc_line;
my @abxab_line1=split/\s+/,$abxab_line;
my @abxac_line1=split/\s+/,$abxac_line;
my @abxcd_line1=split/\s+/,$abxcd_line;
if($abxcc_line1[0]!=0){
